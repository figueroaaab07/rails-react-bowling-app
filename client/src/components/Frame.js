...
function Frame({ frame_number, ball_one_pins, ball_two_pins, ball_three_pins, frame_score, addUpdtFrame, scoreBowling, resultBowling }) {
  ...
  const onBlur = (event) => {
    ...
    const validate = `valid_${name}`;
    if (event.target.innerText.trim() === "" || event.target.innerText.length > 1 || !eval(validate).test(event.target.innerText) || (name === "ball_two_pins" && event.target.innerText === 'X' && frame.frame_number !== 10) || (name === "ball_two_pins" && event.target.innerText !== '/' && (parseInt(event.target.innerText, 10) + parseInt(frame.ball_one_pins, 10)) > 10) || frame.frame_number > scoreBowling.length + 1 || (name === "ball_two_pins" && event.target.innerText === 'X' && frame.frame_number !== 10) || (name === "ball_two_pins" && frame.ball_one_pins === 'X' && frame.frame_number !== 10 && (event.target.innerText !== ''))) {
